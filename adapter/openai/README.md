...
- **Types:** `Translate` / `TranslateTyped` return `*openai.ChatCompletionNewParams`; `ParseResponse(ctx, raw)` expects `*openai.ChatCompletion`; streaming uses `ParseStreamChunk` (or returns `adapter.ErrStreamNotImplemented` if not implemented).
...
- **Helpers:** use `adapter.TextFromParts(parts)` to get text from `[]ContentPart`, and `adapter.ExtractModelConfig(exec.ModelConfig)` for typed model params.
