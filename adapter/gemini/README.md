...
- **Types:** `Translate` / `TranslateTyped` return `*gemini.Request` (Contents and Config); `ParseResponse(ctx, raw)` expects `*genai.GenerateContentResponse`; `ParseStreamChunk` if supported, or `adapter.ErrStreamNotImplemented`.
